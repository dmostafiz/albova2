...
(function ($) {
    ...
    var HHOptions = {
        ...
        init: function () {
            ...
            this.customJob();//-- custom by ohteamvn
        ...
        customJob: function(){
            var cloneButton = $('.hh-options-form[data-tab="#v-pills-pricing_options-tab"] .button-list').parent().clone();
            $('.hh-options-form[data-tab="#v-pills-pricing_options-tab"] .button-list').parent().remove();
            cloneButton.addClass('mt-5');
            $(cloneButton).insertBefore($('#setting-custom_price .price-render'));
        },
        ...
        initTags: function (el) {
            ...
            $('input[data-tag="tags"]', el).each(function () {
                let t = $(this);

                let tagInput = t.get(0),
                    tagData = tagInput.getAttribute('data-tags-whitelist'),
                    tagWhitelist = [];
                tagData = JSON.parse(tagData);

                if (Object.size(tagData) > 0) {
                    for (var iTag = 0; iTag < Object.keys(tagData).length; iTag++) {
                        tagWhitelist.push(tagData[Object.keys(tagData)[iTag]])
                    }
                }
        ...
        initUploadMedia: function (el) {
            ...
            $('.hh-dropzone', el).each(function () {
                ...
                var hhDropzone = new Dropzone(form.get(0), {
                    ...
                    maxFilesize: hh_params.media_upload_size,
                    acceptedFiles: hh_params.media_upload_permission.join(','),
        ...
        mediaRender: function (el, multi, target) {
            let base = this;
            ...
            $('body').append('<div id="hh-media-frame-modal" class="modal fade ' + _class + '" tabindex="-1" role="dialog" aria-hidden="true">\n' +
                ...
                '                            </a></h4>\n' +
                '                    <form class="form form-sm form-search-media" action="' + hh_params.media_modal_search_url + '" method="post"><input type="text" class="form-control" name="search" placeholder="Search ..."></form>' +
                ...
                '                                    <p class="text-muted">\n' +
                '                                        <span>' + hh_params.media_upload_message.type + '</span>\n' +
                '                                        <span>' + hh_params.media_upload_message.size + '</span>\n' +
                '                                    </p>\n' +
                '                                </div>\n' +
                '                            </form>\n' +
                '                        </div>' +
            ...
            mediaModal.on('shown.bs.modal', function (ev) {
                ...
                let data = {
                    ...
                    number: hh_params.media_modal_number_item,
