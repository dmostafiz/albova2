    ...
    let HHActions = {
        ...
        initMultiLanguages: function (el) {
            ...
            $(document.body).off('click', '#hh-language-action .item').on('click', '#hh-language-action .item', function (ev) {
                ...
                //For editor
                let editor = $('.has-translation.hh-editor');
                if (editor.length) {
                    let editor_id = editor.first().attr('id');
                    let editor_name = editor_id.substr(0, editor_id.length - 2);
                    $('textarea#' + editor_name + code).addClass('hh-editor');
                    ...
                    editor.each(function () {
                        ...
                        let editor_code = editor_id.substr(editor_id.length - 2, 2);
                        if (editor_code === code) {
        ...
        initBindData: function (el) {
            $('[data-hh-bind-from]', el).each(function () {
                var t = $(this);
                var target = t.data('hh-bind-from');
                if ($(target).length) {
                    ...
                    el.on('change keyup', target, function () {
                        var val = $(this).val() || t.data('hh-bind-default');
                        t.text(val);
                    });
                }
            });
        ...
        initGlobal: function (el) {
            ...
            $('#api-settings .access-token-preview .copied').click(function (ev) {
                ev.preventDefault();
                $('#access-token-preview-render').trigger('click');
            });

            $('#api-settings .access-token-preview .reset').click(function (ev) {
                ev.preventDefault();

                let button = $(this),
                    container = button.closest('.access-token-preview');

                $.post(button.attr('href'), {
                    '_token': $('meta[name="csrf-token"]').attr('content')
                }, function (respon) {
                    if (typeof respon == 'object') {
                        $('#access-token-preview-render', container).html(respon.access_key);
                    }
                }, 'json');
            });
