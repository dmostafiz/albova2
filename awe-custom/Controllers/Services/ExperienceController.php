...
namespace AweCustom\Controllers\Services;

use App\Http\Controllers\Controller;
use App\Models\Booking;
use App\Models\Comment;
use AweCustom\Models\Experience;
...
class ExperienceController extends Controller
{
    ...
    public function _getSearchResult()
    {
        ...
        $html = '';
        if ($data['total'] > 0) {
            $column = (!isset($post_data['showmap']) || (isset($post_data['showmap']) && $post_data['showmap'] == 'no')) ? 'col-6 col-sm-6 col-md-2' : 'col-6 col-sm-6 col-md-4 col-lg-3 col-xl-3'; //-- custom by ohteamvn
        ...
        if ($data['total'] > 0) {
            foreach ($data['results'] as $k => $v) {
                $locations[$k] = [
                    ...
                    'post_id' => $v->post_id,
                    'title' => get_translate($v->post_title),
                    'url' => get_experience_permalink($v->post_id, $v->post_slug),
    ...
    public function _addToCartExperience(Request $request)
    {
    ...
    public function experienceValidation($data)
    {
        ...
        if ($data['bookingType'] != 'package') {
            ...
            if ($total_people + $total_guest > $max_people) {
                return [
                    'status' => 0,
                    'message' => sprintf(_n(__('This tour is available for %s person'), __('This tour is available for %s people'), $max_people - ($total_people + $total_guest)), $max_people - ($total_people + $total_guest))
                ];
            }
        } else {
    ...
    public function _getTotalPriceExperience(Request $request)
    {
        ...
        $adult_number = (int)request()->get('num_adults', 1);
    ...
    public function _getExperienceAvailabilitySingle(Request $request)
    {
        ...
        if ($startTime && $endTime && hh_compare_encrypt($experienceID, $experienceEncrypt)) {
            ...
            for ($i = $startTime; $i <= $endTime; $i = strtotime('+1 day', $i)) {
                ...
                foreach ($priceItems['results'] as $item) {
                   $date = strtotime(date('Y-m-d', $item->start_date));
                    if ($i == $date) {
                        $in_date = true;
                        $events['events'][] = [
    ...
    public function _updateExperience(Request $request)
    {
        ...
        if ($fields) {
            ...
            if (!empty($data)) {
                ...
                if(isset($data['post_title'])){ //-- custom by ohteamvn
                    $data['post_slug'] = Str::slug(get_translate($data['post_title'])) . '-' . time();
                }
                $experience = new Experience();
