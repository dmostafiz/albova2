...
class Experience extends Model
{
    ...
    public function getSearchResult($data)
    {
        ...
        if(!empty($data['_s'])){//--custom by ohteamvn
            $_s = $data['_s'];
            $sql_etype = DB::table('term')->where('taxonomy_id', 8)
                ->whereRaw("term_title LIKE '%{$_s}%'")
                ->get();
            $edata = [];
            if(count($sql_etype)){
                foreach ($sql_etype as $eitem){
                    array_push($edata, $eitem->term_id);
                }
            }

            if(!empty($edata)){
                $eitem_str = implode(',', $edata);
                $sql->whereRaw("((post_title LIKE '%{$_s}%' OR post_description LIKE '%{$_s}%') OR experience.experience_type IN ({$eitem_str}))");
            }else{
                $sql->whereRaw("(post_title LIKE '%{$_s}%' OR post_description LIKE '%{$_s}%')");
            }
        }
    ...
    {
        $default = [
            'post_title' => 'New Experience - ' . time(),
            'post_slug' => 'new-experience-' . time(),
            'created_at' => time(),
