...
class CarController extends Controller
{
    ...
    public function _addToCartCar(Request $request)
    {
        ...
        if (hh_compare_encrypt($carID, $carEncrypt) && $startDate && $endDate && $number > 0) {
            ...
            if ($is_available['status'] == 0) {
                $this->sendJson([
                    ...
                    'message' => view('common.alert', [
                        'type' => 'danger',
                        'message' => __('The quantity need to less than ' . $carObject->quantity)
                    ])->render()
                ], true);
                ...
                $this->sendJson([
                    ...
                    'message' => view('common.alert', [
                        'type' => 'danger',
                        'message' => __('The number maximum are ' . $is_available['available'])
                    ])->render()
                ], true);
            } else {
        ...
        if ($data['total'] > 0) {
            if ($layout == 'grid') {
                ...
                foreach ($data['results'] as $k => $item) {
                    $html .= view('frontend.car.loop.grid', [
                        'item' => $item,
                ...
                foreach ($data['results'] as $k => $item) {
                    $html .= view('frontend.car.loop.list', [
                        'item' => $item,
                        'class_wrapper' => $class_wrapper
                    ])->render();
                }
        ...
        if ($data['total'] > 0) {
            foreach ($data['results'] as $k => $v) {
                $locations[$k] = [
                    ...
                    'url' => get_car_permalink($v->post_id, $v->post_slug),
                    'thumbnail' => get_attachment_url($v->thumbnail_id, [75, 75]),
                    'address' => get_short_address($v)
                ];
            }
        } else {
