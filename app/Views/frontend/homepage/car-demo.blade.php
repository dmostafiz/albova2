...
<div class="home-page pb-5">
    ...
    <div class="container">
        <!-- Car Types -->
        <?php
        $car_types = get_terms('car-type', true);
        ?>
        @if(count($car_types) > 0)
            <h2 class="h3 mt-4">{{__('Find a Car type')}}</h2>
                    ...
                    <div class="hh-carousel carousel-padding nav-style2"
                        ...
                        <div class="owl-carousel">
                            @foreach($car_types as $item)
                                <?php
                                $url = get_attachment_url($item->term_image, 'full');
                                ?>
                                <div class="item">
                                    <div class="hh-term-item">
                                        <a href="{{ get_car_search_page('?car-type=' . $item->term_id) }}">
                                            <div class="row">
                                                <div class="col-6">
                                                ...
                                                <div class="col-6 d-flex align-items-center">
                                                    <div class="clearfix">
                                                        ...
                                                        <?php
                                                        $car_count = count_car_in_car_type($item->term_id);
                                                        ?>
                                                        <p class="text-muted">{{ sprintf(__('%s Cars'), $car_count) }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </a>
    ...
    <!-- Call to action -->
    <?php
    $page_id = get_option('car_call_to_action_page');
    $cta_background_id = get_option('car_call_to_action_background', '');
    ?>
    @if(!empty($page_id))
        <?php
    ...
    <div class="container">
        <!-- Destination -->
        <?php
        $locations = get_option('car_top_destination');
        ?>
        @if(!empty($locations))
            <h2 class="h3 mt-5">{{__('Top destinations')}}</h2>
            <p>{{__('Most popular destinations')}}</p>
                ...
                <div class="hh-carousel carousel-padding nav-style2"
                    ...
                    <div class="owl-carousel">
                        @foreach($locations as $location)
                            ...
                            $location_url = get_car_search_page();
    ...
    <!-- Car in New York -->
        ...
        ?>
        @if(count($list_services['results']))
            <h2 class="h3 mt-5">{{__('Car in Paris')}}</h2>
            <p>{{__('Book incredible things to do around the world.')}}</p>
            <div class="hh-list-of-services">
                <div class="row">
                    @foreach($list_services['results'] as $item)
                        <div class="col-12 col-md-6 col-lg-3">
                            @include('frontend.car.loop.grid', ['item' => $item])
                        </div>
                    @endforeach
                </div>
            </div>
