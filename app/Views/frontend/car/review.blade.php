...
$comment_number = get_comment_number($post->post_id, 'car');
?>

<div class="home-comment-list mt-3">
    <h3 class="comment-count">
        {{ _n(__('%s review for this car'), __('%s reviews for this car'), $comment_number) }}
    </h3>
...
@if(user_can_review(get_current_user_id(), $post->post_id, 'car'))
    <div class="post-comment parent-form" id="hh-comment-section">
        <div class="comment-form-wrapper">
            <form action="{{ url('add-post-comment') }}" class="comment-form form-sm form-action form-add-post-comment"
                ...
                <input type="hidden" name="comment_type" value="car"/>
                <div class="row">
